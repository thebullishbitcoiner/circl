...
export default function FocusedStatsRow({ eventId, additionalEventIds = [], rCount, allEvents, zaps, reactions, localReposts = [], onOpenZaps, onOpenReactions, onOpenReposts }) {
  const targetIds = [eventId, ...additionalEventIds];
  const kind6Evs = (allEvents || [])
    .filter(e => (e.kind === 6 || e.kind === 16) && e.tags.some(t => t[0] === "e" && targetIds.includes(t[1])));

  const quoteRepostEvs = (allEvents || []).filter(e =>
    e.kind === 1 && !targetIds.includes(e.id) && e.tags.some(t => t[0] === "q" && targetIds.includes(t[1]))
  );

  // localReposts is a backfill ledger (see repostAndQuoteCount in utils.js) that may
  // include reposts/quotes discovered outside `allEvents` (e.g. from non-followed
  // authors) — merge in by id so anything already found in `allEvents` isn't doubled.
  const knownIds = new Set([...kind6Evs.map(e => e.id), ...quoteRepostEvs.map(e => e.id)]);
  const ledgerOnly = (localReposts || []).filter(r => r?.id && !knownIds.has(r.id));

  const repostItems = [
    ...kind6Evs.map(e => ({ type: "repost", pubkey: e.pubkey })),
    ...quoteRepostEvs.map(e => ({
      type: "quote", pubkey: e.pubkey, event: e,
      quotedEvent: (allEvents || []).find(ev => ev.id === eventId) || null,
    })),
    ...ledgerOnly.map(r => ({ type: "repost", pubkey: r.pubkey })),
  ];
