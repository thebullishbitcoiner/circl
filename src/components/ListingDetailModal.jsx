...
export default function ListingDetailModal({ event, profiles, myPubkey, onOpenProfile, publishEvent, onDelete, onUpdated, onClose }) {
  ...
  return (
    <Overlay onDismiss={onClose} centered>
      <div className="action-sheet" style={{ maxWidth: 480, width: "100%", padding: 0, overflow: "hidden", maxHeight: "90vh", display: "flex", flexDirection: "column", borderRadius: 20 }} onClick={e => e.stopPropagation()}>

        {/* Header */}
        <div className="note-header" style={{ padding: "8px 16px", position: "relative" }}>
          <div onClick={e => { e.stopPropagation(); onOpenProfile?.(event.pubkey); onClose(); }} style={{ cursor: "pointer", flexShrink: 0 }}>
        ...
        {isOwn ? (
          ...
          <>
            ...
            <div style={{ display: "flex", gap: 8, padding: "12px 16px 0" }}>
              <button type="button" className="action-sheet-btn highlight-sheet-submit" style={{ flex: 1, justifyContent: "center" }} onClick={handleSave} disabled={busy}>
                {busy ? "Saving…" : "Save"}
              </button>
            </div>
          </>
          ...
          <>
            ...
            <div style={{ padding: "12px 16px 4px", overflowY: "auto", flex: 1, minHeight: 0 }}>
              ...
              {summary && <div style={{ fontSize: 14, color: "var(--text-secondary, var(--text-faint))", marginBottom: 10 }}>{summary}</div>}
              {location && (
