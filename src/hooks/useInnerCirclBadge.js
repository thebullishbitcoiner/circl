...
import { DEFAULT_RELAYS, PLATFORM_PUBKEY, INNER_CIRCL_BADGE_A_TAG, INNER_CIRCL_MEMBER_PUBKEYS } from "../constants.js";
...
// Manually curated membership, until real badge awards (above) are flowing
// from the platform account. See constants.js for the list.
const curatedMemberPubkeys = new Set(INNER_CIRCL_MEMBER_PUBKEYS);
...
export function useIsInnerCircl(pk) {
  useSubscribed();
  return !!pk && (curatedMemberPubkeys.has(pk) || innerCirclAwards.has(pk));
}
...
export function useInnerCirclBadgeYear(pk) {
  ...
  if (curatedMemberPubkeys.has(pk)) return new Date().getFullYear();
  return null;
}
